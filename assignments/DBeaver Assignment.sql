...
###########################################################
DROP DATABASE IF EXISTS SQLProject; #assignment_db
CREATE DATABASE SQLProject; #assignment_db

USE SQLProject; #assignment_db
...
#SELECT * #Useful for the transfer 199.99 problem
FROM customers c 
JOIN accounts_customers ac ON c.customer_id = ac.customer_id 
JOIN accounts a ON ac.account_id = a.account_id 
...
WHERE a.account_id LIKE '900001';

UPDATE accounts a #Put in to Amanda's account
JOIN accounts_customers ac ON a.account_id = ac.account_id 
JOIN customers c ON c.customer_id = ac.customer_id 
SET a.balance = (balance + 199.99)
WHERE a.account_id LIKE '900003'; #Amanda Smith ID

# Change Amanda Smith's last name to "Lastname"
UPDATE customers 
SET name = 'Amanda Lastname' WHERE name LIKE 'Amanda Smith';
